...
- **Effects (`fx`)** – currently limited to `filter`, `delay`, `distortion`, `gain`, and `compressor`, each with the tunable parameters shown in the schema. The compiler preserves extra keys as-is but expects the primary properties per effect.
